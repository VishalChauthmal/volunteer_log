...
describe "Janhour Pages" do
	...
	describe "new" do
		...
		describe "with user signing in" do
			...
			describe "with invalid janhour information" do
			...
			describe "with valid janhour information" do
				before do
					fill_in "Date", with: "2013-01-05" 
			...
			describe "with a date that's in the future" do
				before do
					fill_in "Date", with: Date.parse((Date.today + 1.year).strftime("%Y-01-%d"))
	...
	describe "edit" do
		...
		before do
			sign_in user
			@janhour = user.janhours.create(date: "2013-01-01", numhours: 4.67)
			visit edit_janhour_path(@janhour)
		end
		...
		describe "after editing" do
			...
			it { should have_selector('div.alert.alert-success') }
			it { should have_content('January') }
		end
		...
		describe "with a date that's in the future" do
			before do
				fill_in "Date", with: Date.parse((Date.today + 1.year).strftime("%Y-01-09"))
				fill_in "Number of Hours", with: "6.5"
				click_button "Submit"
			end
	...
	describe "index" do
		...
		before do
			sign_in user
			@janhour = user.janhours.create(date: "#{Date.today.strftime("%Y")}-01-06", numhours: 5.25)
			visit janhours_path
		end
		...
		it { should have_link(@janhour.numhours) }
		it { should have_link('Add') }
		specify { expect(user.janhours.find_by(date: "#{Date.today.strftime("%Y")}-01-07")).to be_nil }
