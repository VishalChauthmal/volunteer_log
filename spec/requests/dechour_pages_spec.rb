...
describe "Dechour Pages" do
	...
	describe "new" do
		...
		describe "with user signing in" do
			...
			describe "with invalid dechour information" do
			...
			describe "with valid dechour information" do
				before do
					fill_in "Date", with: "2013-12-05"
			...
			describe "with a date that's in the future" do
				before do
					fill_in "Date", with: Date.parse((Date.today + 1.year).strftime("%Y-12-%d"))
	...
	describe "edit" do
		...
		before do
			sign_in user
			@dechour = user.dechours.create(date: "2013-12-01", numhours: 4.67)
			visit edit_dechour_path(@dechour)
		end
		...
		describe "after editing" do
			...
			it { should have_selector('div.alert.alert-success') }
			it { should have_content('December') }
		end
...
#		before do
#			sign_in user
#			@dechour = user.dechours.create(date: "#{Date.today.strftime("%Y")}-12-06", numhours: 5.25)
#			visit dechours_path
#		end
...
#		it { should have_link(@dechour.numhours) }
#		it { should have_link('Add') }
#		specify { expect(user.dechours.find_by(date: "#{Date.today.strftime("%Y")}-12-07")).to be_nil }
