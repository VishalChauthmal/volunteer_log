...
describe "Aprhour Pages" do
	...
	describe "new" do
		...
		describe "with user signing in" do
			...
			describe "with invalid aprhour information" do
			...
			describe "with valid aprhour information" do
				before do
			...
			describe "with a date that's in the future" do
				before do
					fill_in "Date", with: Date.parse((Date.today + 1.year).strftime("%Y-04-%d"))
	...
	describe "edit" do
		...
		before do
			sign_in user
			@aprhour = user.aprhours.create(date: "2013-04-01", numhours: 4.67)
			visit edit_aprhour_path(@aprhour)
		end
	...
	describe "index" do
		...
		before do
			sign_in user
			@aprhour = user.aprhours.create(date: "#{Date.today.strftime("%Y")}-04-06", numhours: 5.25)
			visit aprhours_path
		end
		...
		it { should have_link(@aprhour.numhours) }
		it { should have_link('Add') }
		specify { expect(user.aprhours.find_by(date: "#{Date.today.strftime("%Y")}-04-07")).to be_nil }
